...
class _InsertTimePageState extends State<InsertTimePage> {
  ...
  void initState() {
    ...
    if(!_isInserting()){
      this.dateControler.text = this.time.initDate == null ? "" : formatDate.format(DateTime.fromMillisecondsSinceEpoch(this.time.initDate));
      this.initHourController.text = this.time.initDate == null ? "" : formatHour.format(DateTime.fromMillisecondsSinceEpoch(this.time.initDate));
      this.endHourController.text = this.time.endDate == null ? "" : formatHour.format(DateTime.fromMillisecondsSinceEpoch(this.time.endDate));
    }
    else{
  ...
  Future _buttonAction() async {
    ...
    DateTime initDate = initHour == null ? null : new DateTime(date.year, date.month, date.day, initHour.hour, initHour.minute);
    DateTime endDate = endHour == null ? null : new DateTime(date.year, date.month, date.day, endHour.hour, endHour.minute);
    
    Map<String,dynamic> response;
    if(_isInserting()){
      response = await insertTime(initDate, endDate);
      ...
      response = await updateTime(this.time.presenceControlHoursId, initDate, endDate);
      if(this._checkResult(response)){
