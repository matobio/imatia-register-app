...
class AadOAuth {
  ...
  Future<void> _performFullAuthFlow() async {
    ...
    try {
      ...
      Map<String, dynamic> tokenJson = parseUrlParams(code);
      _token =  new Token.fromJson(tokenJson);
    } catch (e) {
      rethrow;
    }
  }

  Map<String,dynamic> parseUrlParams(url) {
    List<String> params = url.split('&');

    Map<String,String> paramarray = new Map();
    paramarray.putIfAbsent('access_token', () => params.removeAt(0).split('=')[1]);
    
    for(String param in params){
      List<String> split = param.split('=');
      paramarray.putIfAbsent(split[0], () => split[1]);
    }
